...
class DigitClassifier(private val context: Context) {
  ...
  private fun classify(bitmap: Bitmap, pixels: FloatArray): String {
    if (!isInitialized) {
  ...
  fun classifyAsync(bitmap: Bitmap, pixels: FloatArray): Task<String> {
    val task = TaskCompletionSource<String>()
    executorService.execute {
      val result = classify(bitmap, pixels)
  ...
  private fun convertBitmapToPixels(bitmap: Bitmap): FloatArray {
    val byteBuffer = ByteBuffer.allocateDirect(modelInputSize)
    byteBuffer.order(ByteOrder.nativeOrder())
